...
const Footer = () => {
  ...
  return (
    <footer className="relative bg-[hsl(220,20%,5%)] text-white border-t border-white/[0.06]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12">
          ...
          {footerSections.map((section) => (
            <div key={section.title}>
              <h4 className="hidden md:block font-heading font-semibold text-xs uppercase tracking-[0.15em] mb-5 text-white/50">
                {section.title}
              </h4>
              <ul className="hidden md:block space-y-3">
                {section.links.map((link) => (
                  <li key={link.label}>
                    <Link to={link.path} className="text-sm text-white/40 hover:text-white transition-colors">
                      {link.label}
                    </Link>
                  </li>
                ))}
              ...
              <div className="md:hidden border-b border-white/[0.06]">
                <button
                  ...
                  className="w-full flex items-center justify-between py-3 text-sm font-semibold uppercase tracking-wider text-white/50"
                >
                  {section.title}
                  <ChevronDown
