  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-heading text-xl font-bold text-navy">Testimonials</h2>
          <p className="text-sm text-muted-foreground mt-0.5">{items.length} review{items.length !== 1 ? "s" : ""}</p>
        </div>
        {!editing && <Button onClick={startNew} size="sm" className="bg-electric hover:bg-electric/90 text-white"><Plus className="w-4 h-4 mr-2" /> Add Review</Button>}
      </div>
